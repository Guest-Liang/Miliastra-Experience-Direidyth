...
// 通用：查找标题文字
const findHeaderTitle = (title: string, contains: any) => {
  const txt = findTextWithinBounds(title, 0, 0, 300, 95, { contains })
  txt?.drawSelf("group_text")
  return txt
}
// 通用：查找底部按钮文字
const findBottomBtnText = (text: string, contains?: any) => {
...
const findBeyondHallBtn = () => {
  const iro = findImageWithinBounds(
    "assets/UI_BtnIcon_Beyond_Hall.png",
    200,
    0,
    150,
    100,
...
const findLeaveRoomBtn = () => {
  const iro = findImageWithinBounds(
    "assets/UI_Icon_Leave_Right.png",
    1570,
    0,
    350,
    100,
...
// 奇域内：检查是否已进入奇域
const findChatBtn = () => {
  return findTextWithinBounds("Enter", 75, 1015, 120, 40, { contains: true })
}
// 存档：查找奇域收藏
...
// 存档：查找要删除的存档位置
const findSaveToDeletePos = (keyword: string) =>
  findTextWithinListView(
    keyword,
    {
      x: 210,
...
// 存档：查找删除局外存档复选框已选中状态
const findDeleteExternalSaveChecked = (colPos: number) => {
  const iro = findImageWithinBounds(
